...
#' }
#' @export
aggregate_ndvi <- function(data_dir, bd_foret_shp, na_flag = 0, max_miss = 0.50, scale_factor = 127.5, filename = "./ndvi_aggregated.tif") {
  lst <- list.files(path = data_dir, pattern = '.tif$', full.names = TRUE)
  r <- terra::rast(lst)
  ...
  d <- as.Date(names(rn), format = "%Y%m%d")
  d_months <- format(d, "%Y-%m")
  ndvi_agg <- terra::tapp(rn, index = d_months, fun = mean, na.rm = TRUE)
  
  terra::writeRaster(ndvi_agg, filename, overwrite = TRUE)
  ndvi_agg
}
...
#' }
#' @export
get_ndvi_diff <- function(ndvi.y, n_years_lag = 1, max_difference_days = 15, min_past_ndvi = 0.7) {
  dates <- as.Date(paste0(gsub("X", "", names(ndvi.y)), ".15"), format = "%Y.%m.%d")
  ndvi_diff <- list()
  
  for (i in seq_along(dates)) {
    date_prev <- dates[i] - (n_years_lag * 365L)
    index_prev <- which(dates - date_prev <= 0 & dates - date_prev > -65)
    index_curr <- which(dates - dates[i] <= 0 & dates - dates[i] > -65)
...
get_cr_mask <- function(ndvi_diff, diff_threshold = -0.5, min_pixels = 20) {
  ...
  # Remove small clusters that are below the minimum pixel threshold
  clusters <- landscapemetrics::patches(cr_mask_clean_merged, directions = 8)
  ...
  # Apply mask to retain only large clusters
  terra::mask(
...
postprocess_cr <- function(cr_mask, min_area = 1000) {
  ...
  clumped_layer <- landscapemetrics::patches(merged_mask, directions = 8)
  
  # Convert clumped raster to polygons and fill holes
  layer_polygons <- terra::as.polygons(clumped_layer, dissolve = TRUE)
  layer_polygons_filled <- landscapemetrics::fillHoles(layer_polygons)
  layer_polygons_filled$dr <- NA
  
  # Add temporal data to each polygon
  for (i in 1:terra::nlyr(cr_mask)) {
    layer <- cr_mask[[i]]
    layer_name <- names(cr_mask)[i]
    active_polygons <- terra::as.polygons(landscapemetrics::patches(layer, directions = 8), dissolve = TRUE)
  ...
  # Project polygons to calculate area and centroid
  layer_polygons_proj <- sf::st_transform(layer_polygons_filled, crs = sf::st_crs(32632))
  polygon_areas <- terra::expanse(layer_polygons_proj, unit = "m")
  polygon_centroids <- terra::centroids(layer_polygons_proj)
  centroid_coords <- sf::st_coordinates(polygon_centroids)
  
  # Filter polygons by area and add metadata
  ...
  # Add date information to each polygon
  date_metadata <- do.call(rbind, lapply(strsplit(gsub("NAX", "", layer_polygons_proj$dr), "X"), function(x) {
    dates <- as.Date(paste0(x, ".01"), format = "%Y.%m.%d")
    min_date <- min(dates)
    max_date <- max(dates)
    mid_date <- as.Date((as.numeric(min_date) + as.numeric(max_date + 30.5)) / 2, origin = "1970-01-01")
    
    list(
      start = format(min_date, "%Y.%m"),
      end = format(max_date, "%Y.%m"),
      mid = format(mid_date, "%Y.%m"),
      year = format(mid_date, "%Y")
    )
  }))
  
  cbind(layer_polygons_proj, date_metadata)
}
...
#' }
#' @export
get_leaflet_map <- function(cr_mask, bd_shp_cropped, comm_geom) {
  pal_cr <- leaflet::colorNumeric("Reds", domain = c(0, 1), na.color = "#00000000")
  ...
  # Popup information for change regions
  popup_cr <- paste0(
    "<strong>Perturbation</strong><br><strong>Period: </strong>", 
    cr_mask$start, " - ", cr_mask$end,
    "<br><strong>Surface (ha): </strong>", round(cr_mask$area_m2 / 1e4, 3),
    "<br><strong>Forest Type: </strong>", cr_mask$Type,
    "<br><strong>Essence: </strong>", cr_mask$ESSENCE
  )
  
  # Popup information for forest types
  popup <- paste0(
    "<strong>Forest Type: </strong>", bd_shp_cropped$Type,
    "<br><strong>Essence: </strong>", bd_shp_cropped$ESSENCE,
    "<br><strong>Surface (ha): </strong>", round(bd_shp_cropped$area / 1e4, 3)
  )
  ...
  leaflet::leaflet() %>%
    ...
    leaflet::addPolygons(
      ...
      color = "white",
      weight = 2,
      options = leaflet::pathOptions(pane = "comm")
    ) %>%
    
    # Add forest type polygons
    leaflet::addPolygons(
      data = bd_shp_cropped,
      fill = ~Type,
      ...
      group = "BD Forêt",
      popup = popup,
      options = leaflet::pathOptions(pane = "BD")
    ) %>%
    
    # Add change region polygons
    leaflet::addPolygons(
      data = cr_mask,
      fillColor = "firebrick",
      fillOpacity = 0.9,
      ...
      group = "Perturbations",
      popup = popup_cr,
      options = leaflet::pathOptions(pane = "perturbations")
    ) %>%
